    ...
    for root, _, files in os.walk(from_path):
        for file in files:
            ...
            if not os.path.exists(to_file):
                print(
                    f"ERROR: TO_FILE {to_file} does not exist! (FROM_FILE: {from_file})")
                continue
            if not filecmp.cmp(from_file, to_file):
                print(
                    f"ERROR: TO_FILE {to_file} is not the same as FROM_FILE {from_file}!")
                continue
...
if __name__ == "__main__":
    ...
    print("Done! :D /reload in-game to load the new scripts!")
